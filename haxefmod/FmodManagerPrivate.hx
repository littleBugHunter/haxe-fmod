...
class FmodManagerPrivate {
	...
	private function CheckIfUpdateIsBeingCalled() {
		...
		if (timeSinceLastUpdate > 1000) {
			trace("Warn: Is FmodManager.Update() in your game loop? It has been "
				+ timeSinceLastUpdate
	...
	function PlaySoundOneShotAtPosition(soundPath:String, position:Vector3D) {
		HaxeFmod.fmod_create_event_instance_one_shot_at(soundPath, position.x, position.y, position.z, 0, 0, 0, 0, 0, -1, 0, -1, 0);
	}
